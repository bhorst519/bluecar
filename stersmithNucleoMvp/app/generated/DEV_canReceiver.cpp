...
float_q CANRX_DEV_GetSFromFrame_TESTER_Servo_Position(const uint8_t * const pData)
{
    ...
    if (   (raw == 8192) // SNA value
        || (__atomic_load_n(pCounter, MEM_ORDER) >= CANRX_DEV_TESTER_request_CYCLE_TIME) )
    {
...
void CANRX_DEV_Run1ms(void)
{
    ...
    {
        uint32_t cycleTimeOverflow_TESTER_request = CANRX_DEV_TESTER_request_CYCLE_TIME + 1U;
        ...
        (void)__atomic_compare_exchange_n(
            ...
            CANRX_DEV_TESTER_request_CYCLE_TIME,
            false,
            MEM_ORDER,
            MEM_ORDER
        );
