...
uint32_q CANRX_PT_GetSFromFrame_TESTER_BrakeLightEnable(const uint8_t * const pData)
{
    ...
    uint32_q converted;
    if (   (raw == 3U) // SNA value
        || (__atomic_load_n(pCounter, MEM_ORDER) >= CANRX_PT_TESTER_request_CYCLE_TIME) )
    {
...
float_q CANRX_PT_GetSFromFrame_VCU_PCBA_Aps1Pct(const uint8_t * const pData)
{
    ...
    if (   (raw == 1023U) // SNA value
        || (__atomic_load_n(pCounter, MEM_ORDER) >= CANRX_PT_VCU_PcbaVitals_CYCLE_TIME) )
    {
        converted = SignalStatus_E::SNA;
    }
    else
...
uint32_q CANRX_PT_GetSFromFrame_VCU_PCBA_BrakeSwitch(const uint8_t * const pData)
{
    ...
    uint32_q converted;
    if (__atomic_load_n(pCounter, MEM_ORDER) >= CANRX_PT_VCU_PcbaVitals_CYCLE_TIME)
    {
        converted = SignalStatus_E::SNA;
    }
    else
...
{
    // This signal belongs to a muxed message but is static. Get contents from latest-received counter
...
void CANRX_PT_Run1ms(void)
{
    ...
    {
        uint32_t cycleTimeOverflow_TESTER_request = CANRX_PT_TESTER_request_CYCLE_TIME + 1U;
        (void)__atomic_add_fetch(&gTESTER_request_RxCounter[i], 1U, MEM_ORDER);
        (void)__atomic_compare_exchange_n(
            &gTESTER_request_RxCounter[i],
            &cycleTimeOverflow_TESTER_request,
            CANRX_PT_TESTER_request_CYCLE_TIME,
            false,
            MEM_ORDER,
            MEM_ORDER
        );
    ...
    {
        uint32_t cycleTimeOverflow_VCU_PcbaVitals = CANRX_PT_VCU_PcbaVitals_CYCLE_TIME + 1U;
        (void)__atomic_add_fetch(&gVCU_PcbaVitals_RxCounter[i], 1U, MEM_ORDER);
        (void)__atomic_compare_exchange_n(
            &gVCU_PcbaVitals_RxCounter[i],
            &cycleTimeOverflow_VCU_PcbaVitals,
            CANRX_PT_VCU_PcbaVitals_CYCLE_TIME,
            false,
            MEM_ORDER,
            MEM_ORDER
        );
